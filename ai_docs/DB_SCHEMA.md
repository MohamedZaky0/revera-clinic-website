...
> **Last Updated:** 2026-09-17 (Reception bot automation tables added — see the end of this file)
...
| `started_at` | timestamptz | nullable. **Added 2026-08-16** by `20260816120000_add_started_at_to_reservations.sql` (RISK-043). Set by the generic `PATCH /api/reservations` update branch the first time `status` transitions to `'started'`, guarded so a later money-only PATCH on an already-started booking does not reset the clock. Before this column existed, `'started'` was the only transition with no time anchor, so a session could sit open indefinitely with nothing to measure staleness against — one was found still open six days later. Not backfilled: for sessions already in flight there is no record of when they began, and inventing one would make a stale session look fresh. |
| `actual_duration_minutes` | integer | nullable. **Added 2026-08-17** by `20260817000000_add_actual_duration_to_reservations.sql` (RISK-043). Set by the generic `PATCH /api/reservations` update branch alongside `completed_at`, computed as `(completed_at - started_at)` in minutes — the real elapsed time for that specific session, distinct from `services.duration_minutes` (the planned/booked duration). Only computed when `started_at` was actually recorded; left `null` rather than guessed for sessions completed without one. Not backfilled, and not yet surfaced anywhere in the UI or reports as of 2026-08-17 — captured for future use. |
...
| Column | Type | Notes |
|---|---|---|
| `id` | bigint | Primary key, identity — **`GENERATED BY DEFAULT`, not `GENERATED ALWAYS`.** The 2026-07-26 dev-schema-baseline migration briefly regressed this to `GENERATED ALWAYS`, which rejects any upsert carrying an explicit `id` (every write to an *existing* service does, via `mapServiceToDb`) with Postgres error 428C9 — silently breaking all Services/Promotions edits for three days since `syncServicesToApi()` only logs a failed sync, no user-facing alert. Fixed by `20260729000000_fix_services_id_identity_generation.sql`. Do not change this back — new-row creation still auto-generates an id when omitted either way. |
...
- `terms_and_conditions` (**corrected 2026-09-23** — this entry previously called it `terms_conditions`, which no code uses; `src/app/api/terms/route.ts` reads and writes `terms_and_conditions`): Stores structured terms and conditions items managed in the admin dashboard and displayed on public `/terms` page. Fields: `id` (uuid), `sort_order` (int), `title_en` (text), `title_ar` (text), `content_en` (text), `content_ar` (text), `link_text_en` (text), `link_text_ar` (text), `link_url` (text), `is_active` (boolean), `created_at` (timestamptz), `updated_at` (timestamptz). Has public read RLS policy.
