...
| 2026-09-24 | **Browser pass** — doctor screen, real UI | dev, disposable patient/package/booking; test superadmin temporarily marked Doctors dept (restored to Reception) | Option 3 shows only "Pulses Used in This Session" + 500/1,000/2,000/5,000 presets. Entering 5,000 against a 3,000 balance shows one amber notice ("Recorded pulses exceed the package balance. Reception will resolve this at checkout."). No package name, balance, price or choice control. | Pass |
| 2026-09-24 | Doctor Complete Treatment (after the unrelated first-visit intake gate) | dev | Clamped consume 3,000 of 5,000 (package `fully_used`); `package_pulse_usage` row 3,000; **0 invoices, 0 packages sold**; one `doctor_session` line "Device — 5000 pulses" at 0 EGP | Pass |
| 2026-09-24 | Reception GET after doctor completion | dev | `deliveredPulses: 0, deficitPulses: 0` — `reservations.delivered_pulses` was NULL | **Failed — RISK-099 #1** |
| 2026-09-24 | After fixing PATCH to persist `delivered_pulses` | dev, fresh booking | `deliveredPulses: 5000, deficitPulses: 2000, consumedForThisReservation: 3000` | Pass |
| 2026-09-24 | Reception drawer for the completed booking | dev, real UI | "✓ Invoice Settled & Paid", no "Pay & Settle Invoice" — deficit prompt unreachable | **Failed — RISK-099 #2** |
| 2026-09-24 | After the `pendingLaserDeficit` fix | dev, real UI | Payment Status "Unpaid" + "Pay & Settle Invoice" button | Pass |
| 2026-09-24 | Checkout modal deficit prompt | dev, real UI | "Package Pulse Deficit — must be resolved before checkout": Delivered 5,000 / Balance 0 / Deficit 2,000; Buy New Package / Pay Per Pulse ("2,000 pulses × 5 EGP = 10,000 EGP") / Resolve Deficit | Pass |
| 2026-09-24 | Confirm & Complete while unresolved | dev, real UI | Blocked: "This booking has an unresolved laser pulse deficit of 2,000 pulses. Resolve it above before completing checkout."; modal stays open | Pass |
...
- [x] Complete the session from the doctor screen. Confirm `package_pulse_usage` has one row for this reservation capped at the old balance (clamped consume, unchanged), and **no** new `invoices`/`invoice_lines` rows and no `New Package:` or `Excess Laser Pulses Deficit` row in `reservation_products` were written by the doctor path. *(Done in the real UI 2026-09-24. Also found and fixed: the doctor PATCH never persisted `delivered_pulses` — RISK-099 #1.)*
