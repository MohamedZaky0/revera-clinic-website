...
| 3B.7 | Doctor commission: `both` option, base, fixed component (**2 duplicate forms**) | 3B.1 | `DONE` | Windsurf | f6bc2cb |
| 3B.8 | Packages admin UI + `/api/packages` CRUD endpoint | 3B.1 | `DONE` | Windsurf | f6bc2cb |
...
**DONE 2026-07-27 — unblocked by resolving RISK-025.** The admin Services screen now loads, saves,
deletes, and reorders services through `/api/services` with bearer-token auth. `duration_minutes` is
returned by the GET mapper, accepted by the POST mapper, and validated server-side as a finite
number `> 0` and `<= 1440` before any DB write. The service modal exposes a numeric "Duration
(minutes)" input that stays in sync with the legacy text dropdown and is included in the upsert
payload. The free-text `duration` column is still written for backward compatibility, but all
capacity and finance calculations should read via `getServiceDurationMinutes()`, which prefers the
numeric value.
...
| 4.4 | Wire the Finance sidebar entry through all 4 permission maps, as a `src/components/admin/Finance/` module | 4.1, 4.2, DEC-027 | `DONE` | Windsurf | a685982 |
