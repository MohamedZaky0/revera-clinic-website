...
- Services Manager (`/admin` Services view) — relocated `Filter` button to the controls bar directly beside the Sort dropdown with an interactive Status filter drawer (`All`, `Active Only`, `Inactive Only`), combined service row status toggle switch and edit button into a single 3-dots (`MoreVertical`) dropdown action menu (`Edit Service`, `Activate/Deactivate`, `Delete Service`), and moved the red Delete button in the Edit Service modal away from the top-right `X` close button to the bottom-left of the modal footer to prevent accidental deletion.
- Inventory & Devices (`/admin` Inventory view) — grouped all device row actions (`Update Pulses`, `Reset Counter`, `View History`, `Edit Device`) into a single compact 3-dots (`MoreVertical`) dropdown menu with outside click dismissal.
