...
- Refined Pending Approvals Table (`src/components/admin/bookings/AdminBookingsView.tsx`) — removed redundant "Branch" subtitle under branch name, left-aligned "Actions" header and button group to match table column alignment, removed patient profile pictures, and made the entire row clickable to open full booking details with `e.stopPropagation()` event handling on action buttons.
