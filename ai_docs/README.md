...
- Customers List (`/admin` Customers view) — removed avatar circle icon and standalone `Phone` & `Email` columns from table, formatted phone number and email address as clean subtitles directly under the Customer name, and updated empty state `colSpan` to 5.
