...
- Admin Bookings view — modular component (`src/components/admin/bookings/AdminBookingsView.tsx`) bound strictly to real database reservations from Supabase (removed hardcoded demo appointments and fake calendar dots, initialized mini-calendar to real current date, rendered live appointments for selected dates, and enabled full-row clickability to open booking details drawer/modal).
...
- Full View New Booking Page — dedicated modular page component (`src/components/admin/bookings/AdminNewBookingView.tsx`) replacing modal popups when clicking New Booking in Admin. Features Branch selection dropdown (pre-populated with active clinic branches), connects to `POST /api/reservations` with direct Supabase fallback (`isManual: true`), receives `dbCustomers` and `branches` arrays from `page.tsx` with a "Browse All Patients" dropdown toggle & live autocomplete search, fallback service title extraction (`s.en || s.name || s.title || s.name_en || s.title_en || s.ar`), dynamic doctor shift time slots with booked slot filtering from `reservations`, active package redemption status from `customer_packages`, appointment summary card, and split submit actions (`Create Booking`, `Create & Print`, and `Create & Send WhatsApp`).
