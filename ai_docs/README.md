...
- Full View New Booking Page — dedicated modular page component (`src/components/admin/bookings/AdminNewBookingView.tsx`) replacing modal popups when clicking New Booking in Admin. Features live patient phone lookup (autofills existing customer details & active package redemption status from `customers` and `customer_packages`), service & doctor selectors, date & time slot selection, session type card toggles, optional notes, real-time appointment summary card, and split submit actions (`Create Booking`, `Create & Print`, and `Create & Send WhatsApp`).
