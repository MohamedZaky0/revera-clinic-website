...
- Full View New Booking Page — dedicated modular page component (`src/components/admin/bookings/AdminNewBookingView.tsx`) replacing modal popups when clicking New Booking in Admin. Connects to `POST /api/reservations` with direct Supabase fallback (`isManual: true`), receives `dbCustomers` array from `page.tsx` with a "Browse All Patients" dropdown toggle & live autocomplete search, fallback service title extraction (`s.en || s.name || s.title || s.name_en || s.title_en || s.ar`), dynamic doctor shift time slots with booked slot filtering from `reservations`, active package redemption status from `customer_packages`, appointment summary card, and split submit actions (`Create Booking`, `Create & Print`, and `Create & Send WhatsApp`).
