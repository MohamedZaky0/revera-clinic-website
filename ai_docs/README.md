...
- Customers List (`/admin` Customers view) — added `whitespace-nowrap` to table header cells (`Customer`, `Phone`, `Email`, `Created At`, `Bookings`, `Active`) to ensure `Created At` header stays on a single clean horizontal line without vertical wrapping.
